...
__attribute__((naked)) void init_scheduler_stack(uint32_t sched_top_of_stack)
{
    __asm volatile("MSR MSP,%0": :"r"(sched_top_of_stack): ); //move from C variable to special register
    __asm volatile("BX LR"); //return main()
}
...
/*
 * Context switch using systick handler
 */
#if 0
...
/*
 * Context switch using pendSV handler
 */
